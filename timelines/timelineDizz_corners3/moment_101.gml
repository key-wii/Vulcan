...
if (global.checkpoint < 24 && enemies >= 25) exit;
else if (global.checkpoint < 27 && enemies >= 35) exit;
else if (enemies >= 45) exit;
...
if (seed == 1) {
	...
	if (sp == 1) {
		...
		spawn2.enemy = obj_enemy_flamer_dizzy;
		spawn3.enemy = obj_enemy_flamer_dizzy;
		if (instance_number(obj_slime_move) < 2)
			spawn4.enemy = obj_enemy_slime;
		else spawn4.enemy = obj_enemy_r_viper_wide;
	}
	if (sp == 2) {
		if (instance_number(obj_slime_move) < 2)
			spawn1.enemy = obj_enemy_slime;
		else spawn1.enemy = obj_enemy_s_viper_wide;
		spawn2.enemy = obj_enemy_slime_dizzy;
		spawn3.enemy = obj_enemy_flamer_dizzy;
		spawn4.enemy = obj_enemy_flamer_dizzy;
	}
	if (sp == 3) {
		spawn1.enemy = obj_enemy_flamer_dizzy;
		if (instance_number(obj_slime_move) < 2)
			spawn2.enemy = obj_enemy_slime;
		else spawn2.enemy = obj_enemy_b_viper_wide;
		spawn3.enemy = obj_enemy_slime_dizzy;
		spawn4.enemy = obj_enemy_flamer_dizzy;
	}
	if (sp == 4) {
		spawn1.enemy = obj_enemy_flamer_dizzy;
		...
		else spawn3.enemy = obj_enemy_r_viper_wide;
		spawn4.enemy = obj_enemy_slime_dizzy;
	}
} else if (seed == 2) {
	...
	else spawn1.enemy =  obj_enemy_r_viper_wide_circle;
	if (instance_number(obj_enemy_mage_viper_wide_circle) < 13)
		spawn2.enemy1 = obj_enemy_mage_viper_wide_circle;
	else spawn2.enemy =  obj_enemy_r_viper_wide_circle;
	if (instance_number(obj_enemy_mage_viper_wide_circle) < 12)
		spawn3.enemy1 = obj_enemy_mage_viper_wide_circle;
	else spawn3.enemy =  obj_enemy_r_viper_wide_circle;
	if (instance_number(obj_enemy_mage_viper_wide_circle) < 11)
		spawn4.enemy1 = obj_enemy_mage_viper_wide_circle;
	else spawn4.enemy =  obj_enemy_r_viper_wide_circle;
} else {
	if (instance_number(obj_enemy_flamer_dizzy) < 5)
		spawn1.enemy1 = obj_enemy_flamer_dizzy;
	else spawn1.enemy =  obj_enemy_b_viper_wide_circle;
	if (instance_number(obj_enemy_flamer_dizzy) < 4)
		spawn2.enemy1 = obj_enemy_flamer_dizzy;
	else spawn2.enemy =  obj_enemy_b_viper_wide_circle;
	if (instance_number(obj_enemy_flamer_dizzy) < 3)
		spawn3.enemy1 = obj_enemy_flamer_dizzy;
	else spawn3.enemy =  obj_enemy_b_viper_wide_circle;
	if (instance_number(obj_enemy_flamer_dizzy) < 2)
		spawn4.enemy1 = obj_enemy_flamer_dizzy;
