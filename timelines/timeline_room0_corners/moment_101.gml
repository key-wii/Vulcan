...
if (seed == 1) {
	...
	if (!instance_exists(obj_enemy_candler_big))
		spawn4.enemy = obj_enemy_candler_big;
	else spawn4.enemy = obj_enemy_candler;
} else if (seed == 2) {
	if (instance_number(obj_enemy_candler_big) > 1)
		spawn1.enemy = obj_enemy_bomb_mom;
	else spawn1.enemy = obj_enemy_b;
	spawn2.enemy = obj_enemy_b;
	...
	spawn3.enemy = obj_enemy_r;
	if (!instance_exists(obj_enemy_miner))
		spawn4.enemy = obj_enemy_miner;
	else spawn4.enemy = obj_enemy_s;
} else if (seed == 4) {
	spawn1.enemy = obj_enemy_s_viper;
	spawn2.enemy = obj_enemy_s_viper;
	if (instance_number(obj_enemy_candler_big) > 1)
		spawn3.enemy = obj_enemy_bomb_mom;
	else spawn3.enemy = obj_enemy_r;
	...
	} else spawn3.enemy = obj_enemy_s_viper;
	if (!instance_exists(obj_enemy_laser)) {
		spawn4.enemy = obj_enemy_laser;
	} else spawn4.enemy = obj_enemy_s_viper;
} else if (seed == 8) {
	...
	} else spawn1.enemy = obj_enemy_s_viper;
	if (!instance_exists(obj_enemy_laser)) {
		spawn2.enemy = obj_enemy_laser;
	} else spawn2.enemy = obj_enemy_flamer;
