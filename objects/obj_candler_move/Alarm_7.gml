...
if(player_exists) {
	...
	if (burnt = 0) radius = 25.1/* + (radiusIncrease * 2)*/;
	if (burnt > 0) radius = 160.1/* + (radiusIncrease * 2)*/;
	if (distance_to_object(obj_tank_pilot) < radius) {
		player_distance = 1;
		go = false;
	} if (distance_to_object(obj_tank_pilot) < radius - 5) {
		player_distance = 2;
	} if (distance_to_object(obj_tank_pilot) < radius - 20) {
		player_distance = 3;
	} if (distance_to_object(obj_tank_pilot) < radius - 24) {
		player_distance = 4;
	} if (distance_to_object(obj_tank_pilot) > radius) {
		go = true;
		player_distance = 0;
