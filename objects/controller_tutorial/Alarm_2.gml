...
if (instance_exists(obj_spawner_tutorial)) {
	with (obj_spawner_tutorial) alarm_set(0, 1);
	if (global.flameKills > 3 && obj_spawner_tutorial.phase == 1) {
		with (obj_spawner_tutorial) phase = 2;
		with (controller_tutorial) phase = 2;
	}
}
...
if (wheels_exist && global.flameKills > 9) obj_tank_move_flame.top_speed_boost = 0;
room_speed = 60;