...
if (moveOverride == false && player_exists == true) {
	...
	mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, spd, obj_wallandenemymoveandenemycontrollerparent_parent);
} if (go = false) {
	//Stops moving if player is too close
	if (player_distance = 1) {
		direction = point_direction(x, y, obj_tank_pilot.x, obj_tank_pilot.y);
	...
	//Backs away if player is even closer, attempting to keep distance
		...
		mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, -2 - backBoost, obj_wallandenemymoveandenemycontrollerparent_parent);
	} else if (player_distance = 3) {
		mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, -4 - backBoost, obj_wallandenemymoveandenemycontrollerparent_parent);
	} else if (player_distance = 4) {
		mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, -5 - backBoost, obj_wallandenemymoveandenemycontrollerparent_parent);
		speedCheckTick = 0;
	} else if (player_distance = 10) {
		mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, -9 - backBoost / 2, obj_wallandenemymoveandenemycontrollerparent_parent);
		speedCheckTick = 0;
	} else if (player_distance = 11) {
		mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, -12 - backBoost / 4, obj_wallandenemymoveandenemycontrollerparent_parent);
		speedCheckTick = 0;
	} else if (player_distance = 12) {
		mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, -15, obj_wallandenemymoveandenemycontrollerparent_parent);
		speedCheckTick = 0;
	} else if (player_distance = 13) {
		mp_potential_step_object(obj_tank_pilot.x, obj_tank_pilot.y, -20, obj_wallandenemymoveandenemycontrollerparent_parent);
		speedCheckTick = 0;
	}
}
}
