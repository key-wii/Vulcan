...
var edgeL = 850 + (sprite_width / 2); var edgeR = room_width - 950 - (sprite_width / 2);
var edgeU = 700 + (sprite_width / 2); var edgeD = room_height - 760 - (sprite_width / 2);
	...
	with (owner) {
		...
		with (slime1) {
			...
			if (childrenLeft == 1) {
				wheels.spd = 9;
				maxHp = 700;
				if (global.level == 9) maxHp = 400;
			} else {
				wheels.spd = 10;
				maxHp = 300;
				if (global.level == 9) maxHp = 100;
			}
			wheels.hp = maxHp;
